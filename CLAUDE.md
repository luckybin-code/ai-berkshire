...
- **배제 대상**: 나무위키 등 위키형 집단창작 사이트, 개인/무명 블로그, 인지도 낮은 군소 인터넷 매체. 이런 출처는 검색 결과에 나오더라도 인용하지 않는다 (사실 확인용 참고조차 지양)
- **우선 사용**: 1차 공시자료(DART), 공식기관 발표(법원 scourt.go.kr, 금융감독원, 통계청 등), 주요 통신사/경제지(연합뉴스, 로이터, 블룸버그, 한국경제, 매일경제 등 오랜 업력의 전국 단위 매체), 증권사 공식 리포트
- 어떤 매체인지 확신이 안 서면 인용하지 말고, WebSearch로 더 신뢰도 높은 대체 출처를 다시 찾는다
- 이 기준은 모든 skill의 리서치 단계(데이터 수집, 뉴스 확인, 배경 조사 등)에 공통 적용된다
